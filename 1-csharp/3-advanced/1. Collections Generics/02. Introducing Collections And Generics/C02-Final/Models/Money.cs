...
public record Money
{
    ...
    public Money Divide(decimal factor) => this with { Amount = this.Amount / factor.NonZero(nameof(factor)) };

    public override string ToString() => $"{this.Amount.ToString(this.AmountFormat)} {this.Currency}";

    private string AmountFormat => "#,##0" + (this.Precision > 0 ? "." + new string('0', this.Precision) : string.Empty);
}